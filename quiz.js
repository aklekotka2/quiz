...
QuizView.prototype.refresh = function () {
	//var pytaniaQuiz=quiz.losujPytania();
	for(var i=0; i<this.pytaniaQuiz.length;i++){
		this.element = document.createElement('li');

		this.trescPytania=document.createTextNode(pytaniaLista.allPytania[this.pytaniaQuiz[i]].tresc);

		this.subul=document.createElement('ul');
		for(var j=0; j<pytaniaLista.allPytania[this.pytaniaQuiz[i]].odpowiedzi.length;j++){
			this.subelement = document.createElement('li');
			this.trescOdpowiedzi=document.createTextNode(pytaniaLista.allPytania[this.pytaniaQuiz[i]].odpowiedzi[j]);
			this.subelement.appendChild(this.trescOdpowiedzi);
			this.subul.appendChild(this.subelement);
		}



		this.element.appendChild(this.trescPytania);


		document.getElementById("myList").appendChild(this.element);
		this.element.appendChild(this.subul);
...
list.appendChild(nowyView.element);